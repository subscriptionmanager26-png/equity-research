...
describe("classifySlackEvent", () => {
  it("treats a reply in a tracked thread as a follow-up even without pocketedge", () => {
    assert.equal(
      classifySlackEvent({
        type: "message",
  ...
  it("treats a pocketedge reply in a tracked thread as a follow-up, not a new job", () => {
    assert.equal(
      classifySlackEvent({
        type: "message",
